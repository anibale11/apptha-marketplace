...
class Saveprofile extends \Magento\Framework\App\Action\Action { 
   /**
    ...
    private function getAbsoluteMediaPath($relativeMediaPath) {
    ...
    public function execute() {
        ...
        if (isset ( $_FILES ['store_banner'] ['name'] ) && $_FILES ['store_banner'] ['name'] != '') {
            ...
            $woshkapath = 'Marketplace/Sellerbanner' . $bannerName;
            $bannerImageResize->open ( $this->getAbsoluteMediaPath($woshkapath) );
    ...
    public function uploadStoreLogo($fileId, $absolutePath) {
        ...
        $woshkapathlogo = 'Marketplace/Sellerlogo' . $logoName;
        $imageResize->open ( $this->getAbsoluteMediaPath($woshkapathlogo) );
