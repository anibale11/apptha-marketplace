...
class System extends \Magento\Framework\App\Helper\AbstractHelper {
    ...
    public function getOffset($start, $end) {
        ...
        for($i = is_countable ( $charsArray ) - 1; $i >= 0; $i --) {
        ...
        $offset = $eNum - $sNum;
        if ($offset < 0) {
            $offset = is_countable ( $charsArray ) + ($offset);
        }
        return $offset;
    }
    /**
    ...
    $objectManager = \Magento\Framework\App\ObjectManager::getInstance ();
        ...
        if ($isSellerSubscriptionEnabled == 1) {
            ...
            if (is_countable ( $sellerSubscribedPlan )) {
                $maximumCount = '';
                ...
                $sellerProductcount = is_countable ( $sellerIdForProducts ) + $productDataTotalCount;
